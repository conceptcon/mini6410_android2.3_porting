					...
					struct v4l2_fmtdesc *f)
{
	...
	//if(index < (sizeof(s3c_fimc_capture_formats)/sizeof(struct v4l2_fmtdesc)))
	{
		memset(f, 0, sizeof(*f));
		memcpy(f, &s3c_fimc_capture_formats[index], sizeof(*f));
	printk("%s index %d, fmt %x  YUYV %x RGB %x\n",__func__,index,f->pixelformat,V4L2_PIX_FMT_YUYV,V4L2_PIX_FMT_RGB565);//hnmsky
	}
	return 0;
}
					...
					struct v4l2_control *c)
{
	...
	case V4L2_CID_JPEG_SIZE:
		c->value = frame->jpeg.main_size;
		break;

	case V4L2_CID_THUMBNAIL_SIZE:
		c->value = frame->jpeg.thumb_size;
		break;
					...
					struct v4l2_buffer *b)
{
	...
	if (b->type != V4L2_BUF_TYPE_VIDEO_OVERLAY && \
		b->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)
		return -EINVAL;
	//printk("mem %x , MMAP %x \n",b->memory ,V4L2_MEMORY_MMAP);//hnmsky
	b->memory = V4L2_MEMORY_MMAP;//hnmsky   this is set to zero in v4l2-ioctl.c
	if (b->memory != V4L2_MEMORY_MMAP)
		return -EINVAL;
	...
	//printk("%s length %d , offset %d \n",__func__,b->length ,b->m.offset);//hnmsky
	return 0;
}
				...
				struct v4l2_buffer *b)
{
	...
	if (b->type != V4L2_BUF_TYPE_VIDEO_CAPTURE &&
		b->type != V4L2_BUF_TYPE_VIDEO_OVERLAY)
		return -EINVAL;
	ctrl->out_frame.cfn = s3c_fimc_get_frame_count(ctrl);
	b->index = ((frame->cfn) % 4);// % frame->nr_frames;//hnmsky ok + 2 here

//	printk("cfn %d nr %d index %d\n",frame->cfn,frame->nr_frames,b->index);//hnmsky
	if (frame->jpeg.enabled) {
		s3c_fimc_stop_dma(ctrl);
		ret = s3c_fimc_encode_jpeg(frame, b->index);
	}
...
#ifdef CONFIG_VIDEO_FIMC_DISABLE_DEPRECATED
