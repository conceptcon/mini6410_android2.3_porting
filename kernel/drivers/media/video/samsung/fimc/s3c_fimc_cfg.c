...
static u32 s3c_fimc_get_buffer_size(int width, int height, enum s3c_fimc_format_t fmt)
{
	...
	printk("%s, buf_size %d \n",__func__,*buf_size);//hnmsky
	return *buf_size;
}
#if 0
int s3c_fimc_alloc_output_memory(struct s3c_fimc_control *ctrl, 
					struct s3c_fimc_out_frame *info)
{
	int ret;

	if (info->jpeg.enabled)
		info->buf_size = JPEG_BUFFER_SIZE;
	else
...
int s3c_fimc_alloc_output_memory(struct s3c_fimc_out_frame *info)
{
	...
	info->buf_size = s3c_fimc_get_buffer_size(info->width, info->height, \
							info->format);
	printk("==== alloc buf_size %d \n",info->buf_size);//hnmsky
	if (info->format == FORMAT_YCBCR420 || info->format == FORMAT_YCBCR422)
		ret = s3c_fimc_alloc_yuv_memory(info);
	else
		ret = s3c_fimc_alloc_rgb_memory(info);
...
static void scale_down_yuv422(char *srcBuf, int srcWidth, int srcHeight, 
				char *dstBuf, int dstWidth, int dstHeight)
{
	int scaleX, scaleY;
	int iXsrc, iXdst;
	int iYsrc;
	...
	for (iYsrc = 0; iYsrc < srcHeight; iYsrc++) {
		if (iYsrc % scaleY == 0) {
			for (iXsrc = iYsrc * srcWidth * 2; \
				iXsrc < iYsrc * srcWidth * 2 + srcWidth * 2;) {
				if ((iXsrc % (4 * scaleX)) == 0) {
...
int s3c_fimc_encode_jpeg(struct s3c_fimc_out_frame *frame, int index)
{
	int thumb_width = 320, thumb_height = 240;
	JPG_ENC_PROC_PARAM jpeg;
	char *scdown;

	switch (frame->jpeg.thumb_res) {
	case THUMB_WQVGA_SIZE:
		thumb_width = 400;
		thumb_height = 240;
		break;

	default:
	case THUMB_QVGA_SIZE:
		thumb_width = 320;
		thumb_height = 240;
		break;

	case THUMB_WQQVGA_SIZE:
		thumb_width = 200;
		thumb_height = 120;
		break;

	case THUMB_QQVGA_SIZE:
		thumb_width = 160;
		thumb_height = 120;
		break;
	}

	scdown = phys_to_virt(s3c_fimc_get_dma_region( \
			thumb_width * thumb_height * 2));

	if (frame->width < thumb_width || frame->height < thumb_height) {
		err("invalid thumb resolution\n");
		return -EINVAL;
	}

	scale_down_yuv422(frame->addr[index].virt_y, \
		frame->width, frame->height, scdown, thumb_width, thumb_height);

	/* copy for main */
	memcpy(phys_to_virt(jpg_data_base_addr) + \
		JPG_STREAM_BUF_SIZE + JPG_STREAM_THUMB_BUF_SIZE, \
		frame->addr[index].virt_y, frame->width * frame->height * 2);

	jpeg.sampleMode = JPG_422;
	jpeg.encType = JPG_MAIN;
	jpeg.quality = frame->jpeg.quality;
	jpeg.width = frame->width;
	jpeg.height = frame->height;

	/* encode for main */
	s3c_jpeg_direct_encode(&jpeg);
	frame->jpeg.main_size = jpeg.fileSize;

	memcpy(frame->addr[index].virt_y, \
		phys_to_virt(jpg_data_base_addr), frame->jpeg.main_size);

	/* encode for thumbnail */
	if (frame->jpeg.thumb) {
		memcpy(phys_to_virt(jpg_data_base_addr) + \
			JPG_STREAM_BUF_SIZE + JPG_STREAM_THUMB_BUF_SIZE, \
			scdown, thumb_width * thumb_height * 2);
		
		jpeg.sampleMode = JPG_422;
		jpeg.encType = JPG_MAIN;
		jpeg.quality = frame->jpeg.quality;
		jpeg.width = thumb_width;
		jpeg.height = thumb_height;

		/* encode for thumbnail */
		s3c_jpeg_direct_encode(&jpeg);
		frame->jpeg.thumb_size = jpeg.fileSize;

		memcpy(frame->addr[index].virt_y + THUMB_OFFSET_JPEG, \
			phys_to_virt(jpg_data_base_addr), frame->jpeg.thumb_size);
	} else {
		memcpy(frame->addr[index].virt_y + (JPEG_BUFFER_SIZE - \
			(thumb_width * thumb_height * 2)), \
			scdown, thumb_width * thumb_height * 2);
	}

	s3c_fimc_put_dma_region(thumb_width * thumb_height * 2);
...
int s3c_fimc_encode_jpeg(struct s3c_fimc_out_frame *frame, int index)
{
	err("jpeg driver must be included for direct encoding\n");
