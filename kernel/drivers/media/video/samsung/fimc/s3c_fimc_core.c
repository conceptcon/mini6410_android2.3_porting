...
#include <linux/slab.h>//hnmsky
#include "s3c_fimc.h"
#define dbg(fmt,arg...) printk("%s:"fmt"\n",__func__, ## arg)//hnmsky
...
void s3c_fimc_register_camera(struct s3c_fimc_camera *cam)
{
	...
	printk("====== camera parent  %x  \n",s3c_fimc.cam_clock->parent);//hnmsky
//return;
...
//	printk("========buf size %d \n",ctrl->out_frame.buf_size);//hnmsky
...
static int s3c_fimc_mmap(struct file* filp, struct vm_area_struct *vma)
{
	...
	printk("the size of mapping is (total_size = %d,size = %d)\n",total_size,size);//hnmsky
	if (size > total_size) 
	{
...
static int s3c_fimc_open(struct file *filp)//hnmsky
{
	struct s3c_fimc_control *ctrl;
	int id, ret;
	//video_drvdata(filp);
	//id = MINOR(inode->i_rdev);
	id = video_devdata(filp)->minor;
	printk("=========%s : id  %d \n",__func__,id);//hnmsky
	ctrl = &s3c_fimc.ctrl[id];
...
static int s3c_fimc_release(struct file *filp)//hnmsky
{
	struct s3c_fimc_control *ctrl;
	int id;

	//id = MINOR(inode->i_rdev);
	id = video_devdata(filp)->minor;//hnmsky
...
static const struct file_operations s3c_fimc_fops = {
	...
	//.ioctl = video_ioctl2,
	.unlocked_ioctl = video_ioctl2,//hnmsky
	.read = s3c_fimc_read,
...
struct video_device s3c_fimc_video_device[S3C_FIMC_MAX_CTRLS] = {
	[0] = {
		//.vfl_type = VID_TYPE_OVERLAY | VID_TYPE_CAPTURE | VID_TYPE_CLIPPING | VID_TYPE_SCALES,//hnmsky
		.fops = &s3c_fimc_fops,
...
static int s3c_fimc_init_global(struct platform_device *pdev)
{
	...
	//s3c_fimc.cam_clock = clk_get(&pdev->dev, "camera");//hnmsky
...
//printk("=============fimc probe \n");//hnmsky
	...
	if (pdata->cfg_gpio)
		pdata->cfg_gpio(pdev);
//printk(" 1src name  %s   name  %s  rate  %d\n",pdata->srclk_name,pdata->clk_name,pdata->clockrate);
	/* fimc source clock */
...
//printk(" 2src name  %s   name  %s\n",pdata->srclk_name,pdata->clk_name);
//printk("================   paret  %x   rate %x\n", ctrl->clock->ops->set_parent,ctrl->clock->ops->set_rate);
/* set parent clock */
...
//
//
//	clk_set_parent(ctrl->clock, srclk);
//	clk_put(srclk);//
//	clk_set_rate(ctrl->clock, pdata->clockrate);
