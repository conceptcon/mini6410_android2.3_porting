...
static int check_fd(struct tslib_input *i)
{
	...
	if (version != EV_VERSION) {
		fprintf(stderr,"version(%x),  EV_VERSION(%x)\n",version, EV_VERSION);
			 ...
			 struct ts_sample *samp, int nr)
{
	...
	unsigned int ts_data = 0;//hnmsky
	int j = 0;
	struct pollfd events[1];
	memset(events,0,sizeof(events[0]));
	events[0].fd = ts->fd;
	...
	if (i->sane_fd == -1)
		return 0;
	fprintf(stderr, "==using_syn (%d)\n",i->using_syn);
	if (i->using_syn) {
#if 0
		int sz=0;
		ts_data = 0;
		sz=read(ts->dev_fd,&ts_data,sizeof(ts_data));//hnmsky
...
#endif
		while (total < nr) {
			fprintf(stderr, "total (%d)   nr(%d)\n",total,nr);
/*
			while(j < 20)
			{
				ret = poll(events,1,500);
				if(ret == 0)
				{
					j++;
					fprintf(stderr, "poll continue j=(%d)\n",j);
				...
				{
					fprintf(stderr, "poll j=(%d)\n",j);

					break;
				}
					fprintf(stderr, "poll timeout j=(%d)\n",j);
...
*/
			...
			case EV_SYN:
				...
				samp->tv = ev.time;
	#ifdef DEBUG
				fprintf(stderr, "syn RAW---------------------> %d %d %d %d.%d\n",
